defmodule Pento.Game.Board do
  ...
  def new(:skewed), do: new(:all, skewed_rect(10, 6))
  def new(:donut) do
  ...
  def won?(%{active_pento: nil, completed_pentos: completed, palette: palette}) do
    length(completed) == length(palette)
  end
  ...
  defp skewed_rect(width, height) do
    for y <- 1..height,
        x <- 1..width do
      {x + y - 1, y}
    end
